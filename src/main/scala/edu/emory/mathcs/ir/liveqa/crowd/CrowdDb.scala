...
import edu.emory.mathcs.ir.liveqa.base.AnswerCandidate.{CROWD, WEB}
...
object CrowdDb extends LazyLogging {
  ...
  /**
    ...
    */
  class Answers(tag: Tag) extends Table[(Int, String, String, String, String, DateTime, Int)](tag, "ANSWERS") {
    ...
    def * = (id, qid, answer, source, worker, created, rank)
  ...
  /**
    ...
    */
  def addAnswer(qid: String, answer: String, source: String, worker: String) = {
    db.run(answers += (0, qid, answer, source, worker, DateTime.now, -1))
  }
  ...
  /**
    ...
    */
  def addAnswers(a: Seq[(String, String, String, String, Int)]) = {
    val futureQuery = db.run(answers ++= a.map(answer => (0, answer._1, answer._2, answer._3, answer._4, DateTime.now, answer._5)))
  ...
  def getRatedAnswers(qid: String): Map[AnswerCandidate, Seq[Int]] = {
    ...
    answerRatings.groupBy(_._1).map {
      case (key, value) =>
        (new AnswerCandidate(WEB, key._3, key._4), value.filter(_._2.isDefined).map(_._2.get._3))
    }
  }
